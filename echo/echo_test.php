...
if ($retval===0 && $nbLignes >= 3) {
    ...
    if (!str_contains($answer, '<?php')) {
        echo("TECHIO> message --channel 'Tips' La balise ouvrante PHP est absente ou mal écrite.\r\n"); 
    } else if (!str_contains($answer, 'echo')) {
        echo("TECHIO> message --channel 'Tips' ✏ Rappelles-toi, tu dois utiliser la commande echo pour afficher du texte.\r\n");  
    } else if (!str_contains($answer, '\n') && $nbLignes > 1) {
        echo("TECHIO> message --channel 'Tips' Penses à utiliser le caractère \\n pour faire un saut de ligne\r\n"); 
    } else if ($nbLignes < 3) {
        echo("TECHIO> message --channel 'Tips' Ajoute au moins " . 3-$nbLignes . " echo pour afficher suffisamment de lignes\r\n");
    }
} else {
    echo("TECHIO> success false  \r\n");
}
